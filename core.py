            ...
            def inner_wrapper(self, *arg, **kwargs):
                for i in range(times):
                    try:
                        ...
                        print("func: {:<15}\tuser: {:<10}\te: \n\n{}".format(func.__name__, self._username, "".join(traceback.format_exception(exc_type, exc_value, exc_tb, limit=2))))
                        if i != times - 1:
                            continue
                        else:
                            if func.__name__ == 'login':
                                return {'code': -1, 'msg': 'error'}
                            # else, _get_*, arg[0]: response
...
class zju():
    ...
    def login_qrcode(self, token):
        ...
        date1 = datetime.datetime.strptime(self._createdate,"%Y%m%d")
        date2 = datetime.datetime.now()
        self._dates = (date2 - date1).days
    ...
    def _rsa_encrypt(self, password_str, e_str, M_str):
        ...
        return hex(result_int)[2:].rjust(128, '0') # int->hex str
    ...
    def go(self, response):
        ...
        try:
            ...
            missing_list = ['{}: miss'.format(i[:5]) if i not in response else '{}: {}'.format(i[:5], response[i]['code']) for i in check_list]
            response['status'] = missing_list
    ...
    def _get_exp(self, response):
        ...
        res = self._get(sess=sess, url='http://sygl.zju.edu.cn/st/exps')
    ...
    def _get_library(self, response):
        ...
        library['most_page'] = {
            'name': book_info[idx2page[0][0]][1],
            'author': book_info[idx2page[0][0]][0],
            'page': idx2page[0][1]
        }
        response['library'] = library
